  ...
  (let [tstyles {:err [[" " :i :cyan] ["✖" :b :red] [" " :i :magenta]]
        ...
        out #(st/re-println
               (st/%** ["  waiting for"]
                       ...
                       ["] " :b :black-bright]
                       [(st/%* (case %1
                                 :err [["q" :b :yellow] [" press: "]
                                       [name :b :blue] [" is not up "]]
                                 :done [[name :b :blue] [" now up - enjoy"]
                                        [" :^) " :b :yellow-bright]]
                                 [[" press "] ["q" :b :yellow]
                                  [" to quit waiting"]]))]))
        done? (atom false)
    ...
    (a/go (let [terminal (-> (TerminalBuilder/builder) tb-system-true tb-build)]
            ...
            (let [reader (->
              ...
              (a/>! vc false))))
    (a/go (loop []
            (let [succ? (try (apply shell shtest)
                             true
...
(defn -main
  [& cli-args]
  (let [{:keys [args opts]}
          (cli/parse-args cli-args {:alias {:n :name} :require [:name]})]
