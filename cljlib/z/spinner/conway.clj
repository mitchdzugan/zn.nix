(ns z.spinner.conway
  (:require [z.unicode :as u]))

(defn board
  ([width height] (board width height 0))
...
(defn neighbours
  ...
  (let [dim (board-shape board)
    ...
    (map (partial get board)
      [(loc [-1 -1]) (loc [0 -1]) (loc [+1 -1]) (loc [-1 0]) #_i (loc [+1 0])
...
(defn transition
  [board]
  (with-meta (mapv (partial cell-transition board) (range (count board)))
    (meta board)))
...
(defn gol-view
  ...
  (let [gol? #(= :x (nth b (gol-pos %1 %2)))
        f1 [(gol? 0 0) (gol? 1 0) (gol? 0 1) (gol? 1 1) (gol? 0 2) (gol? 1 2)]
        f2 [(gol? 2 0) (gol? 3 0) (gol? 2 1) (gol? 3 1) (gol? 2 2) (gol? 3 2)]
        f3 [(gol? 4 0) (gol? 5 0) (gol? 4 1) (gol? 5 1) (gol? 4 2) (gol? 5 2)]]
    [f1 f2 f3]))

(def empty-view
  [(get u/vec-by-block-sextant " ") (get u/vec-by-block-sextant " ")
...
(defn step
  ...
  (let [rng-int #(int (* (rand) %1))
    ...
    (cond (= post-view empty-view)
            (let [glide-x (rng-offsetter 4)
                  ship-x (rng-offsetter 5)
                  ship-y (rng-offsetter 2)]
              (->
                (->>
                  [[(glide-x -1) -2] [(glide-x 0) -1] [(glide-x 0) 0]
                   [(glide-x -1) 0] [(glide-x -2) 0] [(ship-x 5) (ship-y 3)]
                   [(ship-x 5) (ship-y 2)] [(ship-x 5) (ship-y 1)]
                   [(ship-x 6) (ship-y 3)] [(ship-x 6) (ship-y 0)]
                   [(ship-x 7) (ship-y 3)] [(ship-x 8) (ship-y 2)]
                   [(ship-x 8) (ship-y 0)]]
                  (map #(apply gol-pos %1))
